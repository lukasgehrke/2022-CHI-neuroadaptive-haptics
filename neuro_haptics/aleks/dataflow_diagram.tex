...
\begin{figure}[ht]
    ...
    % \node[entity] (bci) at (0,8) {BCI};
    % \node[entity] (eeg) at (8,8) {EEG};
    ...
    % \draw[-{Latex[length=3mm]}, gray] (eeg) -- node[above, text=black] {BrainVision RDA} (bci);
    % \draw[-{Latex[length=3mm]}, gray] (bci) -- node[above left, text=black] {Reward (implicit)} (ai);
    % \draw[-{Latex[length=3mm]}, gray] (unity) -- node[left, text=black] {NAH\_Unity3DEvents} (bci);
    % \draw[-{Latex[length=3mm]}, gray] ([xshift=1cm]unity.north) -- node[above right, text=black] {NAH\_GazeBehavior} (bci.east);
