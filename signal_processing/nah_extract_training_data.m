...
for pID = pIDs
    ...
    % load eye tracker data -> what features to compute and how?
    % what did we show in earlier project to work best?
    ...
    % make it so that object can only be grabbed after first contact
    ...
    chans_ixs = 1:numel(best_chans_ixs);
    chans_scores = sortrows([chans_ixs', best_chans_ixs],2);
    chans_scores = chans_scores(:,1);
    ...
    writematrix(chans_scores, fullfile(path, 'all_chans_score_order.csv'));
